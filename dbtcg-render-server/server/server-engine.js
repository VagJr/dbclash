...
export class GameEngine {
  ...
  _chargeKi(actorKey) {
    ...
    if (actor.ki < 10) {
      ...
      this.fx('kiAura', { attackerKey: actorKey, x: actorKey === 'player' ? 300 : 900, y: 500, color: actor.leader.color });
  ...
  startReactionTimer() {
    ...
    this.reactionTimer = setInterval(() => {
      ...
      this.notifyState(); // Broadcast the tick to clients
  ...
  startBeamClashLoop() {
    ...
    this.beamClashInterval = setInterval(() => {
      ...
      this.notifyState(); // Broadcast beam clash progress to clients
  ...
  getFullSyncState() {
    return {
      ...
      player: {
        ...
        discard: this.player.discard,
        leader: this.player.leader
      },
      opponent: {
        hp: this.opponent.hp,
        maxHp: this.opponent.maxHp,
        ki: this.opponent.ki,
        shields: this.opponent.shields,
        hand: this.opponent.hand,
        deckCount: this.opponent.deck.length,
        discard: this.opponent.discard,
        leader: this.opponent.leader
      }
    };
  }
