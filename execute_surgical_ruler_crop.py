...
# Surgically Measured Ruler Coordinates (100% Exact to 1 Pixel)
...
print('=== EXECUTING SURGICALLY EXACT RULER CROP FOR ALL SHEETS ===')
...
for sheet in sheets_config:
    ...
    for r_idx, row in enumerate(sheet['cards']):
        ...
        for c_idx, card_id in enumerate(row):
            ...
            total_cards += 1
            print(f'  [{total_cards:02d}] {card_id}: exact ruler ({left}, {top}, {left + CARD_W}, {top + CARD_H}) -> Saved!')

print(f'\n>>> {total_cards} CARDS CROPPED WITH SURGICAL PRECISION! <<<')
...
if os.path.exists(leaders_path):
    ...
    for r_idx, row in enumerate(leader_grid):
        ...
        for c_idx, fname in enumerate(row):
            ...
            leader_count += 1
            print(f'  [Leader {leader_count:02d}] {fname}: exact ruler ({left}, {top}, {left + CARD_W}, {top + CARD_H}) -> Saved!')
    ...
    print(f'\n>>> {leader_count} LEADER PORTRAITS CROPPED WITH SURGICAL PRECISION! <<<')

print(f'\n>>> TOTAL PROCESSED: {total_cards} CARDS + {leader_count} LEADERS WITH SURGICAL PRECISION! <<<')
