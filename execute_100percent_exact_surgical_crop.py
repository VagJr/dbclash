...
CARD_W, CARD_H = 175, 228
LEADER_W, LEADER_H = 250, 360

# 100% Verified Surgical Ruler Coordinates (Matches surgical_ruler_100percent.png down to 1 pixel)
COL_LEFTS = [120, 317, 514, 711]
...
print('=== EXECUTING 100% VERIFIED SURGICAL RULER CROP ===')
...
for sheet in sheets_config:
    ...
    for r_idx, row in enumerate(sheet['cards']):
        ...
        for c_idx, card_id in enumerate(row):
            ...
            total_cards += 1
            print(f'  [{total_cards:02d}] {card_id}: surgical crop ({left}, {top}, {left + CARD_W}, {top + CARD_H}) -> Saved!')

print(f'\n>>> {total_cards} CARDS CROPPED WITH 100% SURGICAL PRECISION! <<<')
...
if os.path.exists(leaders_path):
    ...
    for r_idx, row in enumerate(leader_grid):
        ...
        for c_idx, fname in enumerate(row):
            ...
            leader_count += 1
            print(f'  [Leader {leader_count:02d}] {fname}: surgical crop ({left}, {top}, {left + CARD_W}, {top + CARD_H}) -> Saved!')
    ...
    print(f'\n>>> {leader_count} LEADER PORTRAITS CROPPED WITH 100% SURGICAL PRECISION! <<<')

print(f'\n>>> TOTAL PROCESSED: {total_cards} CARDS + {leader_count} LEADERS WITH 100% SURGICAL PRECISION! <<<')
