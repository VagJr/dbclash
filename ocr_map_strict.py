...
leaders = {
    'goku': 'Son Goku (Base)', 'vegeta': 'Vegeta (Príncipe)', 'gohan': 'Gohan (Jovem)',
    'frieza': 'Frieza (Forma Final)', 'piccolo': 'Piccolo (Super Namekiano)', 'trunks': 'Trunks do Futuro (Mestre da Espada)',
...
for path in sorted(glob.glob('c:/dbtcg/assets/cards_tmp/*.png')):
    ...
    print(f'{os.path.basename(path)} -> OCR: "{text}" | BEST: {best_match[0] if best_match else ""}')
