...
sheets = [
  os.path.join(artifacts_dir, 'exact_template_sheet_1_1785054145161.png'),
  os.path.join(artifacts_dir, 'exact_template_sheet_2_1785054172063.png'),
  os.path.join(artifacts_dir, 'exact_template_sheet_3_1785054199092.png')
]
...
print('=== SLICING AND INJECTING CHAINED EXACT TEMPLATE CARD TILES ===')
...
for sheet_path in sheets:
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            print(f'Saved exact template tile for {card_id} -> {target_path}')

print('>>> CHAINED TEMPLATE TILES SLICED & INJECTED WITH 100% VISUAL IDENTITY! <<<')
